...
const FROM_EMAIL = clean(process.env.FROM_EMAIL) || 'Style Haven <onboarding@resend.dev>';
const CUSTOMER_SUPPORT_EMAIL = clean(process.env.CUSTOMER_SUPPORT_EMAIL || process.env.SUPPORT_EMAIL) || 'support@stylehaven.example.com';
...
async function sendEmail({to,subject,html}) {
  if (BREVO_API_KEY) {
    ...
    const sender = fromMatch ? {name: fromMatch[1].trim() || 'Style Haven', email: fromMatch[2].trim()} : {name:'Style Haven', email:FROM_EMAIL};
...
function buildGeneratedProducts(limit){
  ...
  for(let n=0;n<limit;n++){
    ...
    rows.push({id:seq,productId:`SH26-${String(seq).padStart(5,'0')}`,name,category,mainCategory:main,price:basePrice+((n%6)*100),stock:1+(n%5),sizes,sizeGuide:guide,description:`A ${descriptor.toLowerCase()} ${base.toLowerCase()} selected for the Style Haven collection. Designed for easy Indian wardrobes, dependable comfort and repeat wear, with a considered finish that works from everyday plans to occasion dressing.`,fabric:fabric||fabrics[n%fabrics.length],colour,rating:Number((4.2+(n%7)*0.1).toFixed(1)),reviewCount:0,image});
    id++;
  }
  return rows;
}
...
app.post('/api/auth/send-email-otp',async(req,res)=>{
  try{
    ...
    if((purpose==='password-reset'||purpose==='login') && !db.prepare('SELECT 1 FROM users WHERE email=?').get(email)) return res.status(404).json({success:false,error:'No Style Haven account was found with this email.'});
    ...
    const subject=purpose==='signup'?'Verify your Style Haven account':purpose==='login'?'Your Style Haven login code':'Reset your Style Haven password';
    await sendEmail({to:email,subject,html:`<div style="font-family:Arial,sans-serif;line-height:1.6"><h2>Style Haven</h2><p>Your one-time verification code is:</p><div style="font-size:30px;font-weight:700;letter-spacing:8px;margin:18px 0">${otp}</div><p>This code expires in 10 minutes.</p></div>`});
...
app.get('/api/location/reverse', async (req,res)=>{
  ...
  try{
    const r=await fetch(`https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${encodeURIComponent(lat)}&lon=${encodeURIComponent(lon)}&addressdetails=1`,{headers:{'User-Agent':'Style-Haven/1.0 (location lookup)'}});
...
app.get('/api/health',(req,res)=>res.json({ok:true,service:'Style Haven',database:'SQLite',timestamp:now()}));
app.listen(PORT,()=>console.log(`Style Haven running at http://localhost:${PORT}`));
