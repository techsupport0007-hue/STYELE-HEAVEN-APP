    ...
    <div className="group relative overflow-hidden rounded-2xl border border-line bg-white transition-all duration-200 hover:-translate-y-1 hover:shadow-xl hover:shadow-ink/5">
      ...
      <button
        ...
        className={`absolute right-3 top-3 grid h-9 w-9 place-items-center rounded-full text-base shadow-md transition hover:scale-110 ${
          wishlisted ? 'bg-sale text-white' : 'bg-white/95 text-ink'
        }`}
      >
        {wishlisted ? '♥' : '♡'}
      </button>
      ...
      <div className="p-4">
        ...
        <div className="mt-1.5 flex items-baseline gap-2">
          {hasOffer ? (
            <>
              <span className="text-base font-bold text-ink">₹{product.offerPrice}</span>
              <span className="text-[13px] text-muted line-through">₹{product.price}</span>
            </>
          ) : (
        ...
        <p className="mt-1 text-[11px] text-muted">{product.uniqueCode}</p>
        ...
        <button
          ...
          className={`mt-3 h-10 w-full rounded-full text-xs font-bold uppercase tracking-wide transition disabled:opacity-30 ${
            justAdded ? 'bg-emerald text-white' : 'bg-cta text-ink hover:bg-cta-deep'
          }`}
        >
          {!product.stockCount ? 'Out of stock' : justAdded ? 'Added ✓' : 'Add to Cart'}
        </button>
      </div>
    </div>
  );
